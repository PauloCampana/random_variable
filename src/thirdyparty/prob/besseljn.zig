...
const lnGamma = @import("gamma.zig").lnGamma;
const airy = @import("airy.zig").airy;
const besselj0 = @import("besselj0.zig").besselj0;
const besselj1 = @import("besselj1.zig").besselj1;
const polevl = @import("polevl.zig").polevl;
const p1evl = @import("polevl.zig").p1evl;
...
pub fn besselj(n_: f64, x_: f64) f64 {
    ...
    if (y == an) {
        nint = true;
        var i = @as(isize, @intFromFloat(an - 16384.0 * math.floor(an / 16384.0)));
        if (n < 0.0) {
            if (i & 1 != 0) {
                sign = -sign;
            }
...
fn jvs(n: f64, x: f64) f64 {
    ...
    t = frr.significand;
    var ex = @as(f64, @floatFromInt(frr.exponent));
    ex = ex * n;
...
fn hankel(n: f64, x: f64) f64 {
    ...
    u = x - (0.5 * n + 0.25) * constants.PI;
    t = math.sqrt(2.0 / (constants.PI * x)) * (pp * math.cos(u) - qq * math.sin(u));
    return t;
}
...
fn jnx(n: f64, x: f64) f64 {
    ...
    var doa = true;
    var dob = true;
    var akl: f64 = constants.MAXNUM;
    var bkl: f64 = constants.MAXNUM;
    var sign: f64 = 1;
