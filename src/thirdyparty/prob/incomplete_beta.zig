...
pub fn incompleteBeta(aa: f64, bb: f64, xx: f64) f64 {
    ...
    y = a * @log(x);
    var t = b * @log(xc);
    if (a + b < MAXGAM and @abs(y) < constants.MAXLOG and @abs(t) < constants.MAXLOG) {
        t = math.pow(f64, xc, b);
        t *= math.pow(f64, x, a);
        t /= a;
        t *= w;
...
fn incbcf(a: f64, b: f64, x: f64) f64 {
    ...
    var r: f64 = 1;
    var n: f64 = 0;
    const thresh: f64 = 3.0 * constants.MACHEP;
...
fn pseries(a: f64, b: f64, x: f64) f64 {
    ...
    var n: f64 = 2.0;
    var s: f64 = 0.0;
    const z: f64 = constants.MACHEP * ai;
    ...
    u = a * @log(x);
    if ((a + b) < MAXGAM and @abs(u) < constants.MAXLOG) {
        t = gamma(a + b) / (gamma(a) * gamma(b));
        ...
        switch (state) {
            .ihalve => {
                ...
                while (i < 100) : (i += 1) {
                    ...
                    if (y < y0) {
                        ...
                        x1 = x;
                        if (rflg and x1 < constants.MACHEP) {
            ...
            .newton => {
                ...
                while (i < 8) : (i += 1) {
                    ...
                    x = xt;
                    if (@abs(d / x) < 128.0 * constants.MACHEP) {
                        return done(rflg, x);
                    }
                }

                // Did not converge.
                dithresh = 256.0 * constants.MACHEP;
                state = .ihalve;
            },
        }
    }
