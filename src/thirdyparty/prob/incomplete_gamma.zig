...
pub fn incompleteGamma(a: f64, x: f64) f64 {
    ...
    while (true) {
        ...
        if (c / ans <= constants.MACHEP) break;
    }
...
pub fn inverseComplementedIncompleteGamma(a: f64, y0: f64) f64 {
    ...
    // bound the solution
    var x0: f64 = constants.MAXNUM;
    var yl: f64 = 0;
    var x1: f64 = 0;
    var yh: f64 = 1.0;
    const dithresh: f64 = 5.0 * constants.MACHEP;
    ...
    d = 0.0625;
    if (x0 == constants.MAXNUM) {
        if (x <= 0.0) {
            x = 1.0;
        }
        while (x0 == constants.MAXNUM) {
            x = (1.0 + d) * x;
            y = complementedIncompleteGamma(a, x);
            if (y < y0) {
                x0 = x;
